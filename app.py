...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql:///jakedegHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
collections = db.Table('collections',db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')),db.Column('search_id',db.Integer, db.ForeignKey('searches.id')))
collections2 = db.Table('collections2',db.Column('gif_id',db.Integer, db.ForeignKey('gifs.id')),db.Column('personal_id',db.Integer, db.ForeignKey('personals.id')))
...
class User(UserMixin, db.Model):
    ...
    personals = db.relationship('PersonalGifCollection',backref='users')
...
class PersonalGifCollection(db.Model):
    ...
    gifs = db.relationship('Gif',secondary=collections2,backref=db.backref('personals',lazy='dynamic'),lazy='dynamic')
...
class SearchTerm(db.Model):
    ...
    gifs = db.relationship('Gif',secondary=collections,backref=db.backref('searches',lazy='dynamic'),lazy='dynamic')
...
def get_gifs_from_giphy(search_string):
    ...
    parameter = {'q': search_string, 'api_key': api_key, 'limit': '5'}
...
def get_or_create_search_term(term):
    ...
    # This function should invoke the get_gifs_from_giphy function to get a list of gif data from Giphy.
        gifs = get_gifs_from_giphy(search_string=term)
    # It should iterate over that list acquired from Giphy and invoke get_or_create_gif for each, and then append the return value from get_or_create_gif to the search term's associated gifs (remember, many-to-many relationship between search terms and gifs, allowing you to do this!).
        for gif in gifs:
            newgif = get_or_create_gif(title=gif["title"],url=gif["embed_url"])
            search.gifs.append(newgif)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    personal = PersonalGifCollection.query.filter_by(name=name).first()
    if personal:
        return personal
    # In other words, based on the input to this function, if there exists a collection with the input name, associated with the current user, then this function should return that PersonalGifCollection instance.
        ...
        personal = PersonalGifCollection(name=name, user_id=user.id)
        for gif in gif_list:
            gif1 = get_or_create_gif(title=gif.title,url=gif.embedURL)
            personal.gifs.append(gif1)
...
def index():
    ...
    if form.validate_on_submit():
        term = get_or_create_search_term(term=form.search.data)
        url = "/gifs_searched/" + term.term
        return redirect(url)
...
def create_collection():
    ...
    if request.method == 'POST':
        ...
        for val in form.gif_picks.data:
            gif = get_gif_by_id(val)
            gifs.append(gif)
        get_or_create_collection(name=form.name.data,current_user=current_user.username,gif_list=gifs)
